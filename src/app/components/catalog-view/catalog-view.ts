...
export class CatalogViewComponent {
  ...
  filteredMaterials() {
    ...
    if (!term) return list;
    return list.filter(
      (m) => m.description?.toLowerCase().includes(term) || m.code?.toLowerCase().includes(term),
    );
  }
  ...
  filteredLabor() {
    ...
    if (!term) return list;
    return list.filter(
      (l) => l.description?.toLowerCase().includes(term) || l.code?.toLowerCase().includes(term),
    );
  }
  ...
  filteredEquipment() {
    ...
    if (!term) return list;
    return list.filter(
      (e) => e.description?.toLowerCase().includes(term) || e.code?.toLowerCase().includes(term),
    );
  }
  ...
  async createItem() {
    ...
    if (this.activeTab === 'materials') {
      created = await this.catalogService.createMaterial(
        this.newItemName,
        this.newItemUnit,
        this.newItemPrice,
      );
    } else if (this.activeTab === 'labor') {
      created = await this.catalogService.createLabor(
        this.newItemName,
        this.newItemUnit,
        this.newItemPrice,
      );
    } else if (this.activeTab === 'equipment') {
      created = await this.catalogService.createEquipment(
        this.newItemName,
        this.newItemUnit,
        this.newItemPrice,
      );
  ...
  async saveEdit(item: any) {
    ...
    if (this.activeTab === 'apus') {
      // Necesita apuService
      const success = await this.apuService.updateApu(
        item.id,
        this.editItemName,
        this.editItemUnit,
      );
    ...
    const success = await this.catalogService.updateItem(
      type,
      item.id,
      this.editItemName,
      this.editItemUnit,
