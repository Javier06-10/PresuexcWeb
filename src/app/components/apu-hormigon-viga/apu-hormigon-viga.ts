...
@Component({
  selector: 'app-apu-hormigon-viga',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './apu-hormigon-viga.html',
})
export class ApuHormigonVigaComponent implements OnInit, OnChanges {
  @Input() apuParameters: any = {};
  @Output() parametersChange = new EventEmitter<any>();

  tramos      = signal<TramoViga[]>([]);
  desperdicio = signal<number>(5);
  tipoVaciado = signal<string>('In situ');
  aceroKgM3   = signal<number>(100);
  ...
  encofradoM2          = computed(() => this.tramos().reduce((s, t) => s + ((t.largo||0) * (2*(t.altura||0) + (t.ancho||0))) * (t.cant||1), 0));
  ...
  private _loadFromParams() {
    ...
    else this.tramos.set([{ descripcion: 'Viga principal', largo: 0, ancho: 0.20, altura: 0.30, cant: 1 }]);
  ...
  addTramo()             { this.tramos.update(t => [...t, { descripcion: '', largo: 0, ancho: 0.20, altura: 0.30, cant: 1 }]); }
  removeTramo(i: number) { this.tramos.update(t => t.filter((_, idx) => idx !== i)); }
  updateTramo(i: number, field: keyof TramoViga, val: any) {
